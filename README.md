...
- No SSH access: EC2 managed exclusively via AWS Systems Manager (SSM)
- Private service access:	S3 access via VPC endpoint where applicable
- Least-privilege IAM roles:	Lambda, EC2, and API Gateway have scoped permissions
- Backend abstraction:	API Gateway endpoint is not directly exposed to the browser and supports throttling
